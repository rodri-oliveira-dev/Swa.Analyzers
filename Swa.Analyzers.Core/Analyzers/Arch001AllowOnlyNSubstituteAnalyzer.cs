...
public sealed class Arch001AllowOnlyNSubstituteAnalyzer : DiagnosticAnalyzer
{
    ...
    public override void Initialize(AnalysisContext context)
    {
        ...
        context.RegisterCompilationStartAction(startContext =>
        {
            ...
            startContext.RegisterOperationAction(
                operationContext => AnalyzeOperation(operationContext, blockedNamespaces),
    ...
    private static void AnalyzeUsingDirective(SyntaxNodeAnalysisContext context, ImmutableArray<string> blockedNamespaces)
    {
        ...
        {
            var blockedRoot = FindBlockedRoot(namespaceSymbol.ToDisplayString(), blockedNamespaces);
    ...
    private static void AnalyzeOperation(OperationAnalysisContext context, ImmutableArray<string> blockedNamespaces)
    {
        ...
        var blockedRoot = FindBlockedRoot(containingNamespace, blockedNamespaces)
            ?? FindBlockedRoot(symbol.ContainingAssembly?.Name, blockedNamespaces);
    ...
    private static bool IsTestProject(Compilation compilation, ImmutableArray<string> testProjectPatterns)
    {
        ...
        {
            if (ContainsAnyPattern(reference.Name, testProjectPatterns))
            {
                return true;
            }
        }
    ...
    private static string? FindBlockedRoot(string? namespaceOrAssembly, ImmutableArray<string> blockedNamespaces)
    {
